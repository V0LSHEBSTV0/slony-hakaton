...
def evaluate(predicted_file_path: str,
    ...
    try:
        # Загрузка датафреймов напрямую из CSV
        predicted_df = pd.read_csv(predicted_file_path)
        gt_df = pd.read_csv(gt_file_path) if gt_file_path else None

        time_spent = predicted_df['time_spent'].tolist() if 'time_spent' in predicted_df else [0.0] * len(predicted_df)

        # Расчёт метрик
        metric, accuracy, fp_rate, avg_time = compute_overall_metric(
